...
-- =====================================================
-- STEP 1: Clean Trading-Related Data
-- =====================================================

-- Clean IB commissions
TRUNCATE TABLE ib_commissions;

-- Clean swap charges log (if exists)
DELETE FROM swap_charges_log WHERE 1=1;

-- Clean margin events (if exists)
...
-- Clean trading accounts (keep all for now, will clean users later)
-- DELETE FROM trading_accounts WHERE user_id NOT IN (
...
-- =====================================================
-- STEP 4: Insert All Trading Symbols
-- =====================================================

-- Insert all trading symbols with proper configuration
INSERT INTO symbols (
    symbol, 
    name, 
    category_id, 
...
-- =====================================================
-- FOREX MAJORS (Tightest spreads, highest liquidity)
-- =====================================================
...
-- =====================================================
-- FOREX MINORS (Cross pairs, moderate spreads)
-- =====================================================
...
-- =====================================================
-- FOREX EXOTICS (Higher spreads, lower liquidity)
-- =====================================================
...
-- =====================================================
-- STEP 5: Ensure IB Global Settings Exist
-- =====================================================

-- Check if ib_global_settings table exists first
...
INSERT INTO ib_global_settings (setting_key, setting_value, setting_description, is_active)
VALUES
    ('default_commission_rate', 0.0070, 'Default commission rate as percentage (0.70%)', TRUE),
    ('default_ib_share_percent', 50.00, 'Default percentage of commission that IBs receive', TRUE),
    ('min_ib_share_percent', 10.00, 'Minimum IB share percentage', TRUE),
    ('max_ib_share_percent', 90.00, 'Maximum IB share percentage', TRUE)
...
-- =====================================================
-- STEP 6: Ensure Trading Charges Exist
-- =====================================================

-- Check if trading_charges table exists first
...
INSERT INTO trading_charges (symbol_id, charge_type, charge_value, charge_unit, is_active)
VALUES
    (NULL, 'commission', 7.00, 'per_lot', TRUE),
    (NULL, 'carry_forward', 2.50, 'per_lot', TRUE),
    (NULL, 'overnight_fee', 1.00, 'per_lot', TRUE)
ON DUPLICATE KEY UPDATE
...
SELECT 'Database cleanup and reseed completed!' as Status;
SELECT COUNT(*) as 'Total Users' FROM users;
SELECT COUNT(*) as 'Total Symbols' FROM symbols;
SELECT COUNT(*) as 'Active Symbols' FROM symbols WHERE is_active = TRUE;
